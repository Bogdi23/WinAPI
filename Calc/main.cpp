...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		if (LOWORD(wParam) == IDC_BUTTON_POIT)
		{
	...
	case WM_KEYDOWN:
	{
		...
		case VK_OEM_PERIOD:
		case VK_DECIMAL:	SendMessage(GetDlgItem(hwnd, IDC_BUTTON_POIT), BM_SETSTATE, TRUE, 0);	break;
	...
	case WM_KEYUP:
	{
		...
		case VK_OEM_PERIOD:
		case VK_DECIMAL:	
			SendMessage(hwnd, WM_COMMAND, LOWORD(IDC_BUTTON_POIT), 0);	
			SendMessage(GetDlgItem(hwnd, IDC_BUTTON_POIT), BM_SETSTATE, FALSE, 0);	
			break;
		case VK_BACK:		
